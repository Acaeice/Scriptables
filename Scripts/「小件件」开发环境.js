...
class Base {
    ...
    async writeUnicomImage() {
        ...
        imgarr.map(async(v, i) => {
            console.log(v[i]);
            if (!fm.isFileStoredIniCloud(fm.documentsDirectory() + v[i])) {
                console.log(v[i] + "——————————————————————不存在")
                ...
                if (!fm.isFileDownloaded(fm.documentsDirectory() + v[i])) {
                    fm.downloadFileFromiCloud(fm.documentsDirectory() + v[i])
                }
            }
...
(async() => {
    ...
    const req = new Request('https://gitee.com/nodewuzuo/Scriptables/raw/main/package.json')
    ...
    const REMOTE_REQ = new Request('https://gitee.com/nodewuzuo/Scriptables/raw/main/Scripts/%E3%80%8C%E5%B0%8F%E4%BB%B6%E4%BB%B6%E3%80%8D%E5%BC%80%E5%8F%91%E7%8E%AF%E5%A2%83.js')
